...
static inline float pid_rate_update(
    ...
    // 1. LPF na żyroskop (Alpha 0.15 jest OK dla 1kHz)
    ...
    *gyro_state += 0.15f * (rate_meas - *gyro_state);
    ...
    // 5. D-term (D-on-measurement)
    // Obliczamy realną pochodną: zmiana sygnału w czasie
    float D_term = -pid->kd * (*gyro_state - prev_gyro) / dt;
...
static inline float angle_to_rate(float error_angle) {
    ...
    if (abs_err < 0.005f) return 0.0f; // Deadzone ok. 0.3 stopnia

    // Nieliniowość: błąd do potęgi 1.25 (miękki środek, narastająca siła)
    float rate = kp_angle * powf(abs_err, 1.35f); 
    
    if (error_angle < 0) rate = -rate;
...
// ===== FEEDFORWARD =====
float ff_gain = 0.55f;      // 0.2 – 0.6 
float ff_tau  = 0.02f;     // stała czasowa śmigła [s] (20–40 ms)
...
engines calculate_speed_pd(float roll, float pitch, vec3 gyro, float dt)
{
    ...
    // ===== RZECZYWISTE RATE =====
    float roll_rate_meas = predict_rate(gyro.x, roll_rate_target, dt);
    float pitch_rate_meas = predict_rate(gyro.y, pitch_rate_target, dt);
    ...
    rc += rate_feedforward(roll_rate_target, prev_roll_rate_target, dt);
    pc += rate_feedforward(pitch_rate_target, prev_pitch_rate_target, dt);
