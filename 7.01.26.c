...
volatile bool is_flying = false;      // Czy silniki mają prawo pracować?
volatile bool emergency_stop = false; // Czy wymusić natychmiastowe zero?
...
static inline float pid_rate_update(
    ...
    // 1. LPF na żyroskop (zostawiamy 0.15f dla niskiego laga)
    *gyro_state += 0.15f * (rate_meas - *gyro_state);
    ...
    // Surowe D z błędu
    float raw_D = pid->kd * d_error;

    if (raw_D > 12.0f) raw_D = 12.0f;
    if (raw_D < -12.0f) raw_D = -12.0f;

    pid->last += 0.10f * (raw_D - pid->last); 
    float D_term = pid->last;

    if (fabsf(error) < 0.025f) error = 0.0f;
    // 3. P-term
    float P_term = pid->kp * error;

    if (fabsf(rate_target) < 0.70f) { 
        pid->integrator += 0.40f * error * dt;
        pid->integrator *= 0.99998f; 
    } 
    ...
    for_debug = I_term;
    // 5. NOWY D-term (Pochodna z błędu)
    // d_error pokazuje jak szybko zbliżamy się lub oddalamy od celu
    

    // 6. Sumowanie
    float out = P_term + I_term + D_term;

    // 7. Rate limit (Slew rate)
    ...
    // Zapis stanu dla następnej iteracji
    pid->prev_output = out;
...
engines calculate_speed_pd(float roll, float pitch, vec3 gyro, float dt)
{
    ...
    if (emergency_stop || !is_flying) {
        // Natychmiastowe zatrzymanie wszystkich wyjść PWM
    ...
    switch (c) {
        case 'g': // START
            if (!is_flying) {
                ...
                is_flying = true;
                target_alt = 30.0f; // Ustaw wysokość startową
                // Resetujemy regulatory, żeby nie "strzeliły" starymi wartościami
                alt_pid.integrator = 0;
        ...
        case 'q': // STOP (Emergency)
        case ' ': // Spacja też jako stop
            is_flying = false;
            emergency_stop = true;
            target_base_motor = 0;
            base_motor = 0; // Pominięcie rampy
            uart_last_char = 0;
            break;

        // Sterowanie kierunkowe (teraz bez sleep_ms - dron leci dopóki nie puścisz lub nie dasz 'x')
        case 'w': base_pitch = -0.09f; break;
        case 's': base_pitch = 0.09f; break;
        case 'a': base_roll = -0.09f; break;
        case 'd': base_roll = 0.09f; break;
        case 'x': base_pitch = 0; base_roll = 0; break; 
...
void steer_by_altitude_step(float target_alt_cm) {
    ...
    if (!is_flying || emergency_stop) {
        target_base_motor = 0;
        alt_pid.integrator = 0;
        return;
    }

    // Wykonuj PID tylko gdy dron leci
    float corr = altitude_pid_update(&alt_pid, target_alt_cm, alt_filt, 0.05f);
    float new_base = (float)hover_throttle + corr;
...

{
    ...
    while(true){
        ...
        printf("iterm: %3.3f\n", pid_pitch.ki * pid_pitch.integrator);   
