...
static inline float pid_rate_update(
    ...
    *gyro_state += 0.15f * (rate_meas - *gyro_state);
    ...
    // TŁUMIK UDERZEŃ: Ograniczamy max wpływ D-term, żeby nie "stukał" w silniki
    // Wartość 40-50 jest bezpieczna dla out_limit 180
    if (raw_D > 12.0f) raw_D = 12.0f;
    if (raw_D < -12.0f) raw_D = -12.0f;

    // Filtr LPF na D-term (zmniejszony do 0.10 dla lepszego wygładzenia d_error)
