...
pid_t pid_roll = {40.0f, 0.4f, 3.0f, 0.0f, 0.0f, 0.0f};
pid_t pid_pitch = {40.0f, 0.4f, 3.0f, 0.0f, 0.0f, 0.0f};
...
static inline float pid_update(pid_t *pid, float error, float gyro_rate, float dt)
{
    ...
    if (fabsf(gyro_rate) < 0.05f) {          // ~3 deg/s
        pid->bias += 0.6f * error * dt;       // bias_gain = 0.6
    }
