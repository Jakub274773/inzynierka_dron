...
static inline float pid_rate_update(
    ...
    *gyro_state += 0.15f * (rate_meas - *gyro_state);
    ...
    // 4. Inteligentny I-term (wykorzystujemy error do warunku)
    // Jeśli błąd jest mniejszy niż 0.15 rad/s (dron blisko celu), budujemy całkę.
    // Jeśli błąd jest duży (manewr), przestajemy zwiększać integrator.
    if (fabsf(rate_target) < 0.70f) { 
        pid->integrator += 1.2 * error * dt;
        //pid->integrator *= 0.999f;

    } 
    
    // Leaky Integrator - zapobiega długofalowemu "puchnięciu"
