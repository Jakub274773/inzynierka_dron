...
pid_t pid_roll = {0.015f, 0.0f, 0.010f, 0.0f, 0.0f, 0.0f};
pid_t pid_pitch = {0.015f, 0.0f, 0.010f, 0.0f, 0.0f, 0.0f};

static inline float pid_update(pid_t *pid, float error, float gyro_rate, float dt)
{
    /* === D-term z żyroskopu === */
    float d = (error - pid->last)/0.002f;
    pid->last = error;
...
engines calculate_speed_pd(float roll,float pitch, vec3 gyro, float dt){
    ...
    if(rc > 4)rc = 4; if(rc < -4)rc = -4;
    if(pc > 4)pc = 4; if(pc < -4)pc = -4;
    ...
    if(roll_corr>corr_limit)roll_corr=corr_limit; if(roll_corr<-corr_limit)roll_corr=-corr_limit;
    if(pitch_corr>corr_limit)pitch_corr=corr_limit; if(pitch_corr<-corr_limit)pitch_corr=-corr_limit;

     printf("RC: %3.3f, PC: %3.3f\n", rc, pc);
     printf("RCorr: %3.3f, PCorr: %3.3f\n", roll_corr, pitch_corr);
    float fa = base_motor - roll_corr + pitch_corr + frac_a;
    float fb = base_motor - roll_corr - pitch_corr + frac_b;
    float fc = base_motor + roll_corr + pitch_corr + frac_c;
    float fd = base_motor + roll_corr - pitch_corr + frac_d;
