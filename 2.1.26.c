...
void ekf_rp_update_acc(
    ...
    if (fabsf(norm - 1.0f) > 0.125f) return; // gating
...
// ANGLE loop
float kp_angle = 1.2f;        // rad/s per rad
float max_rate = 0.5f;        // rad/s
...
{
    if(fabs(roll) < trim_angle_thresh) roll_trim = 0;
    else roll_trim += trim_gain * roll * dt;

    if(fabs(pitch) < trim_angle_thresh) pitch_trim = 0;
    else pitch_trim += trim_gain * pitch * dt;
